...
type UploadInterface interface {
	...
	Start() Upload
	Init(config *cf.AutoGenerated) Upload
}
...
func (h Func) Init(config *cf.AutoGenerated) Upload {
	cosRegion := utils.SplitCosBucketUrl(config.Bucket.BucketURL)
	var Upload Upload = &Conf{
		sId:        config.Bucket.Secretid,
		sKey:       config.Bucket.Secretkey,
		buckUrl:    config.Bucket.BucketURL,
		serviceUrl: cosRegion,
	}
	return Upload
}
